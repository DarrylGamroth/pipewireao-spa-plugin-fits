...
#define MAX_BUFFERS ANDOR3_CAMERA_MAX_BUFFERS
...
struct buffer_slot {
	...
	struct andor3_camera_buffer *camera_buffer;
	uint32_t id;
	bool camera_queued;
	bool published;
};
		...
		const char *model, uint32_t device_index)
{
	...
	offset = (size_t)snprintf(destination, capacity, "andor3_source.%" PRIu32 ".",
			device_index);
...
static int map_format(const struct andor3_camera_info *info,
		uint32_t *format)
{
		...
		uint32_t start, uint32_t num, const struct spa_pod *filter)
{
	...
	while (count < num) {
		...
		if (id == SPA_PARAM_PropInfo)
			param = andor3_build_feature_prop_info(self->camera,
					result.index, &dynamic.b);
		else if (result.index == 0)
			param = andor3_build_feature_props(self->camera, &dynamic.b);
		else
			param = NULL;
		if (param == NULL)
			break;
...
static int parse_feature_value(enum andor3_feature_kind kind,
		const struct spa_pod *pod, struct andor3_feature_value *value)
{
	uint32_t id;
		...
		uint32_t flags SPA_UNUSED, const struct spa_pod *param)
{
	...
	if (self->started)
		return -EBUSY;
	if ((res = andor3_camera_find_feature(self->camera, name,
			&feature_index)) < 0 ||
			(res = andor3_camera_get_feature_info(self->camera, feature_index,
			&info)) < 0)
		return res;
	if (!info.available)
		return -ENODATA;
	...
	if ((res = parse_feature_value(info.kind, value_pod, &value)) < 0)
		return res;
	if (spa_streq(info.name, "PixelEncoding")) {
		const char *entry;
		struct andor3_camera_info candidate = self->camera_info;
		uint32_t format;

		if (value.enumeration < 0 ||
				(uint32_t)value.enumeration >= info.n_enum_entries ||
				(entry = andor3_camera_get_feature_enum_entry(self->camera,
				feature_index, (uint32_t)value.enumeration)) == NULL)
			return -EINVAL;
		snprintf(candidate.pixel_encoding, sizeof(candidate.pixel_encoding),
				"%s", entry);
...
static int queue_slot(struct impl *self, struct buffer_slot *slot)
{
	...
	if ((res = andor3_camera_queue(self->camera, slot->camera_buffer)) < 0)
		return res;
...
static int stop_source(struct impl *self)
{
	...
	if (!self->started)
		return 0;
	res = andor3_camera_stop(self->camera);
	for (i = 0; i < self->port.n_buffers; i++)
...
static int node_send_command(void *object, const struct spa_command *command)
{
	...
	case SPA_NODE_COMMAND_Start:
		...
		if ((res = queue_producer_buffers(self)) < 0) {
			(void)andor3_camera_stop(self->camera);
			for (i = 0; i < self->port.n_buffers; i++)
				self->slots[i].camera_queued = false;
			return res;
		}
		if ((res = andor3_camera_start(self->camera)) < 0) {
			(void)andor3_camera_stop(self->camera);
			for (i = 0; i < self->port.n_buffers; i++)
				self->slots[i].camera_queued = false;
			return res;
...
static int release_buffers(struct impl *self)
{
	...
	for (i = 0; i < self->port.n_buffers; i++) {
		...
		if (slot->camera_buffer != NULL &&
				(res = andor3_camera_revoke(self->camera,
		...
		struct spa_buffer **buffers, uint32_t n_buffers)
{
	...
	for (i = 0; i < n_buffers; i++) {
		...
		slot->id = i;
		if ((res = andor3_camera_announce(self->camera, data->data,
...
error:
	for (i = 0; i < announced; i++)
		if (self->slots[i].camera_buffer != NULL)
			(void)andor3_camera_revoke(self->camera,
...
static int publish_completion(struct impl *self,
		const struct andor3_camera_completion *completion)
{
...
static int node_process(void *object)
{
	...
	if ((res = recycle_buffer(self)) < 0)
		return res;
	if ((res = andor3_camera_try_get_completion(self->camera,
			&completion)) < 0)
		return res;
	if (res == 0)
		return SPA_STATUS_OK;
...
static int clear(struct spa_handle *handle)
{
	...
	if (self->port.n_buffers != 0 &&
		...
		first_error = res;
	andor3_camera_close(self->camera);
	self->camera = NULL;
	return first_error;
}
...
#define ADD_ITEM(key, value) \
	...
	ADD_ITEM(SPA_KEY_DEVICE_API, "andor3");
	...
	ADD_ITEM(SPA_KEY_DEVICE_VENDOR_NAME, "Andor");
	...
	ADD_ITEM(SPA_KEY_API_ANDOR3_DEVICE_INDEX, self->device_index);
	ADD_ITEM(SPA_KEY_API_ANDOR3_READINESS, "poll");
	ADD_ITEM(SPA_KEY_API_ANDOR3_PIXEL_ENCODING,
...
{
	struct impl *self = (struct impl *)handle;
	struct andor3_camera_options options = { 0 };
	const struct andor3_camera_info *camera_info;
	const char *value;
	int res;
	...
	value = info == NULL ? NULL :
			spa_dict_lookup(info, SPA_KEY_API_ANDOR3_DEVICE_INDEX);
	if (value != NULL && !spa_atou32(value, &options.device_index, 0))
		return -EINVAL;
	value = info == NULL ? NULL :
			spa_dict_lookup(info, SPA_KEY_API_ANDOR3_READINESS);
	if (value != NULL && !spa_streq(value, "poll"))
		return -EINVAL;
	spa_hook_list_init(&self->hooks);
	if ((res = andor3_camera_open(&self->camera, &options)) < 0)
		return res;
	camera_info = andor3_camera_get_info(self->camera);
...
const struct spa_handle_factory spa_andor3_source_factory = {
	SPA_VERSION_HANDLE_FACTORY,
	SPA_NAME_API_ANDOR3_SOURCE,
	NULL,
	get_size,
	init,
	enum_interface_info,
