...
struct test_buffer {
	...
	_Alignas(8) uint8_t payload[FRAME_SIZE];
};
		...
		bool discontinuity)
{
	...
	spa_assert_se(buffer->chunk.stride == 8);
...
int main(int argc, char *argv[])
{
	struct spa_dict_item items[] = {
		SPA_DICT_ITEM_INIT(SPA_KEY_API_HAMAMATSU_DEVICE_INDEX, "0"),
	};
	struct spa_dict info = SPA_DICT_INIT(items, SPA_N_ELEMENTS(items));
	...
	spa_assert_se(spa_streq(capture.pixel_encoding, "Mono12"));
	{
		...
		struct spa_pod_int mono8 = SPA_POD_INIT_Int(0);
		struct spa_pod_int mono12 = SPA_POD_INIT_Int(1);
		...
		spa_assert_se(spa_streq(property_name, "hamamatsu.Width"));
		props = enum_node_one(node, &capture, SPA_PARAM_Props, 0);
		value = find_control_value(props, "hamamatsu.ExposureTime");
		spa_assert_se(value != NULL);
		write = build_control_write(write_storage, sizeof(write_storage),
				"hamamatsu.ExposureTime", value);
		spa_assert_se(spa_node_set_param(node, SPA_PARAM_Props, 0, write) == 0);
		write = build_control_write(write_storage, sizeof(write_storage),
				"hamamatsu-command.SoftwareTrigger", &bool_value.pod);
		spa_assert_se(spa_node_set_param(node, SPA_PARAM_Props, 0, write) ==
				-ENODATA);
		...
		write = build_control_write(write_storage, sizeof(write_storage),
				"hamamatsu.PixelFormat", &mono8.pod);
		spa_assert_se(spa_node_set_param(node, SPA_PARAM_Props, 0, write) == 0);
		spa_assert_se(spa_format_video_raw_parse(enum_one(node, &capture,
				SPA_PARAM_EnumFormat), &video) >= 0);
		spa_assert_se(video.format == SPA_VIDEO_FORMAT_GRAY8);
		write = build_control_write(write_storage, sizeof(write_storage),
				"hamamatsu.PixelFormat", &mono12.pod);
		spa_assert_se(spa_node_set_param(node, SPA_PARAM_Props, 0, write) == 0);
		write = build_control_write(write_storage, sizeof(write_storage),
				"hamamatsu.Width", &long_value.pod);
		spa_assert_se(spa_node_set_param(node, SPA_PARAM_Props, 0, write) == 0);
		spa_assert_se(spa_format_video_raw_parse(enum_one(node, &capture,
				SPA_PARAM_EnumFormat), &video) >= 0);
		spa_assert_se(video.size.width == 5 && video.size.height == 3);
		write = build_control_write(write_storage, sizeof(write_storage),
				"hamamatsu.Width", &reset_width.pod);
		spa_assert_se(spa_node_set_param(node, SPA_PARAM_Props, 0, write) == 0);
	}
	spa_assert_se(spa_format_video_raw_parse(enum_one(node, &capture,
			SPA_PARAM_EnumFormat), &video) >= 0);
	...
	{
		struct spa_pod_bool bool_value = SPA_POD_INIT_Bool(true);
		uint8_t write_storage[256];
		struct spa_pod *write = build_control_write(write_storage,
				sizeof(write_storage), "hamamatsu-command.SoftwareTrigger",
				&bool_value.pod);
