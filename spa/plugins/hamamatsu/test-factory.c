...
int main(int argc, char *argv[])
{
	...
	spa_assert_se(factory != NULL &&
			spa_streq(factory->name, SPA_NAME_API_HAMAMATSU_SOURCE));
	...
	if (!real_sdk) {
		const struct spa_dict_item items[] = {
			SPA_DICT_ITEM_INIT(SPA_KEY_API_HAMAMATSU_DEVICE_INDEX, "1"),
		};
		const struct spa_dict info = SPA_DICT_INIT(items,
				SPA_N_ELEMENTS(items));
	...
	{
		const struct spa_dict_item items[] = {
			SPA_DICT_ITEM_INIT(SPA_KEY_API_HAMAMATSU_DEVICE_INDEX, "invalid"),
		};
		const struct spa_dict info = SPA_DICT_INIT(items,
				SPA_N_ELEMENTS(items));
	...
	{
		const struct spa_dict_item items[] = {
			SPA_DICT_ITEM_INIT(SPA_KEY_API_HAMAMATSU_READINESS, "eventfd"),
		};
		const struct spa_dict info = SPA_DICT_INIT(items,
				SPA_N_ELEMENTS(items));
