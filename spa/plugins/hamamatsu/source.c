...
#define MAX_BUFFERS HAMAMATSU_CAMERA_MAX_BUFFERS
...
struct buffer_slot {
	...
	struct hamamatsu_camera_buffer *camera_buffer;
	uint32_t id;
	bool camera_queued;
	bool published;
};
...
static void make_node_name(char *destination, size_t capacity,
		const char *model, uint32_t device_index)
{
	size_t i, offset;

	offset = (size_t)snprintf(destination, capacity, "hamamatsu_source.%" PRIu32 ".",
			device_index);
	for (i = 0; model[i] != '\0' && offset + 1u < capacity; i++) {
...
static int map_format(const struct hamamatsu_camera_info *info,
		uint32_t *format)
{
	if (spa_streq(info->pixel_encoding, "Mono8") &&
			info->stride >= info->width) {
		*format = SPA_VIDEO_FORMAT_GRAY8;
		return 0;
	}
	if ((spa_streq(info->pixel_encoding, "Mono12") ||
			spa_streq(info->pixel_encoding, "Mono16")) &&
			info->width <= UINT32_MAX / 2u &&
			info->stride >= info->width * 2u) {
		...
		uint32_t start, uint32_t num, const struct spa_pod *filter)
{
	...
	while (count < num) {
		...
		if (id == SPA_PARAM_PropInfo)
			param = hamamatsu_build_feature_prop_info(self->camera,
					result.index, &dynamic.b);
		else if (result.index == 0)
			param = hamamatsu_build_feature_props(self->camera, &dynamic.b);
		else
			param = NULL;
		if (param == NULL)
			break;
...
static int parse_feature_value(enum hamamatsu_feature_kind kind,
		const struct spa_pod *pod, struct hamamatsu_feature_value *value)
{
	uint32_t id;
...
static int read_layout(struct impl *self, struct hamamatsu_camera_info *candidate,
		uint32_t *format)
{
	const struct hamamatsu_camera_info *info;
	int res;

	if ((res = hamamatsu_camera_refresh_info(self->camera)) < 0)
		return res;
	info = hamamatsu_camera_get_info(self->camera);
...
{
	struct hamamatsu_camera_info candidate;
	uint32_t format;
	int res;
...
static bool layout_matches(const struct hamamatsu_camera_info *a,
		const struct hamamatsu_camera_info *b)
{
...
static int restore_layout_feature(struct impl *self, uint32_t feature_index,
		const struct hamamatsu_feature_value *old_value)
{
	struct hamamatsu_camera_info restored;
	uint32_t format;
	int res;

	if ((res = hamamatsu_camera_set_feature_value(self->camera, feature_index,
			old_value)) < 0 ||
		...
		uint32_t flags SPA_UNUSED, const struct spa_pod *param)
{
	...
	struct hamamatsu_feature_value value, old_value;
	...
	if (operations == 0)
		return 0;
	if ((res = hamamatsu_camera_find_feature(self->camera, name,
			&feature_index)) < 0 ||
			(res = hamamatsu_camera_get_feature_info(self->camera, feature_index,
			&info)) < 0)
		return res;
	if (!info.available)
		return -ENODATA;
	if (!info.writable)
		return -EACCES;
	if (self->started && info.kind != HAMAMATSU_FEATURE_COMMAND)
		return -EBUSY;
	if (info.changes_layout && self->port.n_buffers != 0)
		return -EBUSY;
	...
	if (spa_streq(info.name, "PixelFormat")) {
		...
		if (value.enumeration < 0 ||
				...
				(entry = hamamatsu_camera_get_feature_enum_entry(self->camera,
					feature_index, (uint32_t)value.enumeration)) == NULL)
			return -EINVAL;
		if (!spa_streq(entry, "Mono8") && !spa_streq(entry, "Mono12") &&
				!spa_streq(entry, "Mono16"))
			return -ENOTSUP;
	}
	if (info.changes_layout) {
		if (!info.readable || (res = hamamatsu_camera_get_feature_value(self->camera,
				feature_index, &old_value)) < 0)
			return !info.readable ? -EACCES : res;
		if (old_value.kind == HAMAMATSU_FEATURE_STRING) {
...
static int queue_slot(struct impl *self, struct buffer_slot *slot)
{
	...
	if ((res = hamamatsu_camera_queue(self->camera, slot->camera_buffer)) < 0)
		return res;
...
static int node_send_command(void *object, const struct spa_command *command)
{
	...
	case SPA_NODE_COMMAND_Start:
		...
		if ((res = queue_producer_buffers(self)) < 0) {
			(void)hamamatsu_camera_stop(self->camera);
			for (i = 0; i < self->port.n_buffers; i++)
				self->slots[i].camera_queued = false;
			return res;
		}
		if ((res = hamamatsu_camera_start(self->camera)) < 0) {
			(void)hamamatsu_camera_stop(self->camera);
			for (i = 0; i < self->port.n_buffers; i++)
				self->slots[i].camera_queued = false;
			return res;
		...
		struct spa_pod_builder *builder, struct spa_pod **param)
{
	...
	case SPA_PARAM_Buffers:
		...
		*param = spa_pod_builder_add_object(builder,
				...
				SPA_POD_Int((int32_t)self->camera_info.stride),
...
static int release_buffers(struct impl *self)
{
	...
	for (i = 0; i < self->port.n_buffers; i++) {
		...
		if (slot->camera_buffer != NULL &&
				(res = hamamatsu_camera_revoke(self->camera,
		...
		struct spa_buffer **buffers, uint32_t n_buffers)
{
	...
	for (i = 0; i < n_buffers; i++) {
		...
		slot->id = i;
		if ((res = hamamatsu_camera_announce(self->camera, data->data,
...
error:
	for (i = 0; i < announced; i++)
		if (self->slots[i].camera_buffer != NULL)
			(void)hamamatsu_camera_revoke(self->camera,
...
static int publish_completion(struct impl *self,
		const struct hamamatsu_camera_completion *completion)
{
	...
	frame = (struct pwao_image_frame) {
		...
		.stride = (int32_t)self->camera_info.stride,
...
static int node_process(void *object)
{
	...
	if ((res = recycle_buffer(self)) < 0)
		return res;
	if ((res = hamamatsu_camera_try_get_completion(self->camera,
			&completion)) < 0)
		return res;
	if (res == 0)
		return SPA_STATUS_OK;
...
static int clear(struct spa_handle *handle)
{
	...
	if (self->port.n_buffers != 0 &&
		...
		first_error = res;
	hamamatsu_camera_close(self->camera);
	self->camera = NULL;
	return first_error;
}
		...
		const struct hamamatsu_camera_options *options)
{
	...
	snprintf(self->node_description, sizeof(self->node_description),
			"%.180s (Hamamatsu DCAM device %" PRIu32 ")",
			self->camera_info.model, options->device_index);
...
#define ADD_ITEM(key, value) \
	...
	ADD_ITEM(SPA_KEY_DEVICE_VENDOR_NAME, "Hamamatsu Photonics");
...
{
	struct impl *self = (struct impl *)handle;
	struct hamamatsu_camera_options options = { 0 };
	const struct hamamatsu_camera_info *camera_info;
	const char *value;
	int res;
	...
	value = info == NULL ? NULL :
			spa_dict_lookup(info, SPA_KEY_API_HAMAMATSU_DEVICE_INDEX);
	if (value != NULL && !spa_atou32(value, &options.device_index, 0))
		return -EINVAL;
	value = info == NULL ? NULL :
	...
	if ((res = hamamatsu_camera_open(&self->camera, &options)) < 0)
		return res;
	camera_info = hamamatsu_camera_get_info(self->camera);
	if (camera_info == NULL || camera_info->payload_size > INT32_MAX ||
			camera_info->image_size > UINT32_MAX ||
			camera_info->stride > INT32_MAX ||
...
const struct spa_handle_factory spa_hamamatsu_source_factory = {
	SPA_VERSION_HANDLE_FACTORY,
	SPA_NAME_API_HAMAMATSU_SOURCE,
	NULL,
	get_size,
	init,
	enum_interface_info,
