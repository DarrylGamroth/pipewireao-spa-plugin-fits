...
static bool is_scalar(enum aravis_feature_kind kind)
{
	return kind != ARAVIS_FEATURE_COMMAND;
}

static int scalar_feature_at(struct aravis_camera *camera,
		uint32_t scalar_index, uint32_t *feature_index,
		struct aravis_feature_info *info)
{
	uint32_t count = aravis_camera_get_feature_count(camera);
	uint32_t i;

	for (i = 0; i < count; i++) {
		if (aravis_camera_get_feature_info(camera, i, info) < 0 ||
				!is_scalar(info->kind))
			continue;
		if (scalar_index-- == 0) {
			*feature_index = i;
...
static void append_value(struct spa_pod_builder *builder,
		const struct aravis_feature_value *value)
{
	switch (value->kind) {
	case ARAVIS_FEATURE_BOOLEAN:
...
{
	struct aravis_feature_info info;
	struct aravis_feature_value current;
	...
	have_value = info.available && info.readable &&
			aravis_camera_get_feature_value(camera, index, &current) == 0;
	...
	case ARAVIS_FEATURE_INTEGER: {
		...
		if (info.available && aravis_camera_get_feature_integer_range(camera,
	...
	case ARAVIS_FEATURE_FLOATING: {
		...
		if (info.available && aravis_camera_get_feature_float_range(camera,
				index, &minimum, &maximum) == 0)
	...
	case ARAVIS_FEATURE_ENUMERATION: {
		...
		for (i = 0; i < info.n_enum_entries; i++) {
			const char *entry = aravis_camera_get_feature_enum_entry(camera,
					index, i);
			spa_pod_builder_int(builder, (int32_t)i);
		...
		struct spa_pod_builder *builder)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (aravis_camera_get_feature_info(camera, i, &info) < 0 ||
				...
				aravis_camera_get_feature_value(camera, i, &value) < 0)
			continue;
