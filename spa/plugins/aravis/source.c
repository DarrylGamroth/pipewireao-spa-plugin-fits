...
struct impl {
	...
	struct spa_dict_item prop_items[9];
	char node_name[192];
		...
		uint32_t flags SPA_UNUSED, const struct spa_pod *param)
{
	...
	if (info.changes_layout) {
		if (!info.readable || (res = aravis_camera_get_feature_value(this->camera,
...
static int release_buffers(struct impl *this)
{
	...
	for (i = 0; i < this->port.n_buffers; i++) {
		...
		if (slot->camera_buffer != NULL &&
				(res = aravis_camera_revoke(this->camera,
		...
		const struct aravis_camera_options *options)
{
	...
	snprintf(this->device_id, sizeof(this->device_id), "%s", options->device_id);
		...
		const struct spa_support *support, uint32_t n_support)
{
	...
	options.device_id = info == NULL ? NULL :
	...
	if (camera_info == NULL || camera_info->payload_size > INT32_MAX ||
			...
			camera_info->width > INT32_MAX / this->bytes_per_pixel) {
		res = -ENOTSUP;
		goto error;
	}
