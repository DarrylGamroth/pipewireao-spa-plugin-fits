...
#define MAX_BUFFERS FLISDK_CAMERA_MAX_BUFFERS
...
struct buffer_slot {
	...
	struct flisdk_camera_buffer *camera_buffer;
	uint32_t id;
	bool camera_queued;
	bool published;
};
...
struct impl {
	...
	char grabber_name[256];
	char pixel_sign[16];
	char control_name[16];
	char clprotocol_libraries[1024];
	char clprotocol_device[256];
	char camera_serial[256];
	char genapi_runtime[1024];
	char control_timeout_ms[16];
	struct port port;
	struct flisdk_camera *camera;
#ifdef HAVE_CLPROTOCOL_CONTROL
	struct flisdk_control *control;
#endif
	struct flisdk_camera_info camera_info;
...
static void make_node_name(char *destination, size_t capacity,
		const char *camera_name)
{
	size_t i, offset;

	offset = (size_t)snprintf(destination, capacity, "flisdk_source.");
	for (i = 0; camera_name[i] != '\0' && offset + 1u < capacity; i++) {
		unsigned char c = (unsigned char)camera_name[i];

		destination[offset++] = (c >= 'a' && c <= 'z') ||
				(c >= 'A' && c <= 'Z') || (c >= '0' && c <= '9') ||
				c == '_' || c == '-' || c == '.' ? (char)c : '_';
	}
...
static int map_format(const struct flisdk_camera_info *info,
		uint32_t *format)
{
		...
		uint32_t start, uint32_t num, const struct spa_pod *filter)
{
	...
	if (self->control == NULL ||
			(id != SPA_PARAM_PropInfo && id != SPA_PARAM_Props))
		return -ENOENT;
	...
	while (count < num) {
		...
		if (id == SPA_PARAM_PropInfo)
			res = flisdk_control_build_prop_info(self->control, result.index,
					&dynamic.b, &param);
		else if (result.index == 0)
			res = flisdk_control_build_props(self->control, &dynamic.b, &param);
		else
			res = 0;
		if (res <= 0)
			break;
		...
		uint32_t flags SPA_UNUSED, const struct spa_pod *param)
{
	...
	if (!spa_pod_is_object(param) ||
		...
		return -EINVAL;
	{
		struct spa_pod_object *props = (struct spa_pod_object *)param;
	...
	if (self->started)
		return -EBUSY;
	res = flisdk_control_set(self->control, name, value,
			self->port.n_buffers == 0, &layout_changed);
	if (res < 0)
		return res;
	if (flisdk_camera_refresh_info(self->camera) < 0)
		return -EIO;
	self->camera_info = *flisdk_camera_get_info(self->camera);
	if (map_format(&self->camera_info, &self->video_format) < 0)
		return -ENOTSUP;
	snprintf(self->pixel_sign, sizeof(self->pixel_sign), "%s",
			self->camera_info.unsigned_pixels ? "unsigned" : "signed");
...
static int queue_slot(struct impl *self, struct buffer_slot *slot)
{
	...
	if ((res = flisdk_camera_queue(self->camera, slot->camera_buffer)) < 0)
		return res;
...
static int stop_source(struct impl *self)
{
	...
	if (!self->started)
		return 0;
	res = flisdk_camera_stop(self->camera);
	for (i = 0; i < self->port.n_buffers; i++)
...
static int node_send_command(void *object, const struct spa_command *command)
{
	...
	case SPA_NODE_COMMAND_Start:
		...
		if ((res = queue_producer_buffers(self)) < 0)
			return res;
		if ((res = flisdk_camera_start(self->camera)) < 0)
			return res;
		self->started = true;
...
static int release_buffers(struct impl *self)
{
	...
	for (i = 0; i < self->port.n_buffers; i++) {
		...
		if (slot->camera_buffer != NULL &&
				(res = flisdk_camera_revoke(self->camera,
		...
		struct spa_buffer **buffers, uint32_t n_buffers)
{
	...
	for (i = 0; i < n_buffers; i++) {
		...
		slot->id = i;
		if ((res = flisdk_camera_announce(self->camera, data->data,
...
error:
	for (i = 0; i < announced; i++)
		if (self->slots[i].camera_buffer != NULL)
			(void)flisdk_camera_revoke(self->camera,
...
static int publish_completion(struct impl *self,
		const struct flisdk_camera_completion *completion)
{
...
static int node_process(void *object)
{
	...
	if ((res = recycle_buffer(self)) < 0)
		return res;
	if ((res = flisdk_camera_try_get_completion(self->camera,
			&completion)) < 0)
		return res;
	if (res == 0)
		return SPA_STATUS_OK;
...
static int clear(struct spa_handle *handle)
{
	...
	if (self->port.n_buffers != 0 &&
		...
		first_error = res;
#ifdef HAVE_CLPROTOCOL_CONTROL
	flisdk_control_close(self->control);
	self->control = NULL;
#endif
	flisdk_camera_close(self->camera);
	self->camera = NULL;
	return first_error;
}
...
#define ADD_ITEM(key, value) \
	...
	ADD_ITEM(SPA_KEY_DEVICE_VENDOR_NAME, "First Light Imaging");
	ADD_ITEM(SPA_KEY_DEVICE_PRODUCT_NAME, self->camera_info.model);
	ADD_ITEM(SPA_KEY_API_FLISDK_CAMERA, self->camera_name);
	if (options->grabber_name != NULL)
		ADD_ITEM(SPA_KEY_API_FLISDK_GRABBER, self->grabber_name);
	ADD_ITEM(SPA_KEY_API_FLISDK_PIXEL_SIGN, self->pixel_sign);
	ADD_ITEM(SPA_KEY_API_FLISDK_READINESS, "poll");
	ADD_ITEM(SPA_KEY_API_FLISDK_CONTROL, self->control_name);
	if (self->clprotocol_libraries[0] != '\0')
		ADD_ITEM(SPA_KEY_API_FLISDK_CLPROTOCOL_LIBRARIES,
				self->clprotocol_libraries);
	if (self->clprotocol_device[0] != '\0')
		ADD_ITEM(SPA_KEY_API_FLISDK_CLPROTOCOL_DEVICE,
				self->clprotocol_device);
	if (self->camera_serial[0] != '\0')
		ADD_ITEM(SPA_KEY_API_FLISDK_CAMERA_SERIAL, self->camera_serial);
	if (self->genapi_runtime[0] != '\0')
		ADD_ITEM(SPA_KEY_API_FLISDK_GENAPI_RUNTIME, self->genapi_runtime);
	ADD_ITEM(SPA_KEY_API_FLISDK_CONTROL_TIMEOUT_MS, self->control_timeout_ms);
#undef ADD_ITEM
...
{
	struct impl *self = (struct impl *)handle;
	struct flisdk_camera_options options = { 0 };
#ifdef HAVE_CLPROTOCOL_CONTROL
	struct flisdk_control_options control_options = { .timeout_ms = 1000 };
#endif
	const struct flisdk_camera_info *camera_info;
	const char *value;
	int res;
	...
	value = info == NULL ? NULL :
			spa_dict_lookup(info, SPA_KEY_API_FLISDK_CAMERA);
	if (copy_text(self->camera_name, sizeof(self->camera_name), value) < 0)
		return -EINVAL;
	value = info == NULL ? NULL :
			spa_dict_lookup(info, SPA_KEY_API_FLISDK_GRABBER);
	if (value != NULL) {
		if (copy_text(self->grabber_name, sizeof(self->grabber_name), value) < 0)
			return -EINVAL;
		options.grabber_name = self->grabber_name;
	}
	value = info == NULL ? NULL :
			spa_dict_lookup(info, SPA_KEY_API_FLISDK_READINESS);
	if (value != NULL && !spa_streq(value, "poll"))
		return -EINVAL;
	options.camera_name = self->camera_name;
	if (copy_text(self->control_name, sizeof(self->control_name),
			info == NULL ? NULL : spa_dict_lookup(info,
					SPA_KEY_API_FLISDK_CONTROL)) < 0)
		snprintf(self->control_name, sizeof(self->control_name), "none");
	if (!spa_streq(self->control_name, "none") &&
			!spa_streq(self->control_name, "clprotocol"))
		return -EINVAL;
	snprintf(self->control_timeout_ms, sizeof(self->control_timeout_ms), "1000");
#ifdef HAVE_CLPROTOCOL_CONTROL
#define READ_CONTROL_TEXT(key, member) do { \
	value = info == NULL ? NULL : spa_dict_lookup(info, (key)); \
	if (value != NULL && copy_text(self->member, sizeof(self->member), value) < 0) \
		return -EINVAL; \
} while (0)
	READ_CONTROL_TEXT(SPA_KEY_API_FLISDK_CLPROTOCOL_LIBRARIES,
			clprotocol_libraries);
	READ_CONTROL_TEXT(SPA_KEY_API_FLISDK_CLPROTOCOL_DEVICE, clprotocol_device);
	READ_CONTROL_TEXT(SPA_KEY_API_FLISDK_CAMERA_SERIAL, camera_serial);
	READ_CONTROL_TEXT(SPA_KEY_API_FLISDK_GENAPI_RUNTIME, genapi_runtime);
#undef READ_CONTROL_TEXT
	value = info == NULL ? NULL : spa_dict_lookup(info,
			SPA_KEY_API_FLISDK_CONTROL_TIMEOUT_MS);
	if (value != NULL) {
		char *end = NULL;
		...
		if (value[0] == '\0' || end == NULL || *end != '\0' || parsed == 0 ||
				parsed > UINT32_MAX ||
				copy_text(self->control_timeout_ms,
					sizeof(self->control_timeout_ms), value) < 0)
			return -EINVAL;
		control_options.timeout_ms = (uint32_t)parsed;
	}
	control_options.libraries = self->clprotocol_libraries[0] == '\0' ? NULL :
			self->clprotocol_libraries;
	control_options.device_template = self->clprotocol_device[0] == '\0' ? NULL :
			self->clprotocol_device;
	control_options.camera_serial = self->camera_serial[0] == '\0' ? NULL :
			self->camera_serial;
	control_options.genapi_runtime = self->genapi_runtime[0] == '\0' ? NULL :
			self->genapi_runtime;
	if (spa_streq(self->control_name, "none") &&
			(self->clprotocol_libraries[0] != '\0' ||
			 self->clprotocol_device[0] != '\0' ||
			 self->camera_serial[0] != '\0' ||
			 self->genapi_runtime[0] != '\0'))
		return -EINVAL;
#else
	if (spa_streq(self->control_name, "clprotocol") ||
			(info != NULL &&
			 (spa_dict_lookup(info,
					SPA_KEY_API_FLISDK_CLPROTOCOL_LIBRARIES) != NULL ||
			  spa_dict_lookup(info,
					SPA_KEY_API_FLISDK_CLPROTOCOL_DEVICE) != NULL ||
			  spa_dict_lookup(info, SPA_KEY_API_FLISDK_CAMERA_SERIAL) != NULL ||
			  spa_dict_lookup(info, SPA_KEY_API_FLISDK_GENAPI_RUNTIME) != NULL)))
		return -ENOTSUP;
#endif
	spa_hook_list_init(&self->hooks);
	if ((res = flisdk_camera_open(&self->camera, &options)) < 0)
		return res;
	camera_info = flisdk_camera_get_info(self->camera);
...
#ifdef HAVE_CLPROTOCOL_CONTROL
	if (spa_streq(self->control_name, "clprotocol") &&
			(res = flisdk_control_open(&self->control, self->camera,
					&control_options)) < 0)
		goto error;
#endif
	...
	if (
		...
		self->info_all |= SPA_NODE_CHANGE_MASK_PARAMS;
	}
...
const struct spa_handle_factory spa_flisdk_source_factory = {
	SPA_VERSION_HANDLE_FACTORY,
	SPA_NAME_API_FLISDK_SOURCE,
	NULL,
	get_size,
	init,
	enum_interface_info,
