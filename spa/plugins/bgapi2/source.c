...
#define MAX_BUFFERS BGAPI2_CAMERA_MAX_BUFFERS
...
static int take_recycled_buffer(struct impl *this,
		struct buffer_slot **recycled)
{
...
static int impl_node_process(void *object)
{
	...
	struct buffer_slot *recycled = NULL;
	...
	res = take_recycled_buffer(this, &recycled);
	if (res < 0)
		return res;
	...
	if (recycled != NULL && (queue_res = queue_slot(this, recycled)) < 0 &&
			res >= 0)
		res = queue_res;
	if (res < 0)
		return res;
