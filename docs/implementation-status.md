...
| CAL-009 | Execute a complete raw-frame-to-ALPAO reference system. | Latest-buffer frame ingress, pixel calibration, fused Shack-Hartmann controller, ALPAO normalization, and latest-buffer sink | `spa-calculon-full-system` negotiates every downstream edge and checks both boundary leases, numerical activity, exact vector conversion, ALPAO acceptance, 468-actuator operation, and zero warmed algorithm allocations; the same harness passes ASDK `SIM001`. `spa-fits-calculon-ingress` qualifies an actual FITS source at the entrance. | Verified |
...
| CAL-005 | Preserve complete-frame publication under standard and latest-buffer transport. | Standard reserve/commit/publish sequence plus bounded latest-buffer leases on streaming inputs and outputs | C ABI and full-system tests verify input completion, output publication, chunk layout, values, Header propagation, and both latest-buffer boundaries. | Verified |
