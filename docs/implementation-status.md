...
| CAMERA-005 | Quiesce an RTC camera source before its final link and announced buffers are dismantled. | Requires a PipeWireAO RTC-node lifecycle correction; it is not a vendor-plugin migration concern. | The full eGrabber host harness currently terminates the daemon with `SIGSEGV` after the final subscriber leaves. GDB shows the main thread waiting in `pw_rtc_data_loop_stop()` while the RTC thread is in eGrabber `processEventFilter()` and a Gigelink worker faults. | Open |
